...
public class FileUtils {
    public List<FileDto> parseReviewFileInfo(int tableNum, int pkNum, MultipartHttpServletRequest multiUploadFiles) throws Exception {
...
//      반환 타입에 맞는 데이터객체를 생성(=DB에 저장될 내용을 담을 List 타입의 변수)
        ...
        String path = "///C:/images/" + current.format(format);                  // 실제 파일이 저장될 폴더명
...
//                  DB 에 저장할 수 있는 BoardFileDto 클래스 타입의 객체를 생성하고 파일정보를 저장함
                    ...
                    boardFile.setStoredFilePath(current.format(format) + "/" + newFileName);              // 서버에 저장될 경로
...
//    실제 파일이 저장될 폴더명, 현재날짜를 기준으로 하면 'images/20220518' 이라는 폴더를 생성함
        String path = "///C:/images/" + current.format(format);
...
//          DB에 저장할 수 있는 BoardFileDto 클래스 타입의 객체를 생성하고 파일 정보를 저장함
                    ...
                    File.setTableNum(tableNum); // 현재 게시판 번호 저장
                    File.setPkNum(pkNum); // 현재 게시물의 글번호를 저장
                    File.setFileSize(multipartFile.getSize()); // 파일의 크기 저장
                    File.setOriginalFileName(multipartFile.getOriginalFilename()); // 원본 파일의 파일명을 저장
                    // File.setStoredFilePath(path + "/" + newFileName); // 서버에 저장될 경로, 위에서 생성한 폴더의 경로와 System.nanoTime을 사용하여 생성한 고유한 파일명을 합하여 실제 파일이 저장되는 전체 경로를 저장함
                    File.setStoredFilePath(current.format(format) + "/" + newFileName);
